...
const CommentSection: React.FC = () => {
  ...
  useEffect(() => {
    const fetchComments = async () => {
      try {
        const response = await axios.get(`http://127.0.0.1:5000/story/${params}`);
  ...
  const handleAddComment = async () => {
    try {
      const response = await axios.post(`http://127.0.0.1:5000/story/${params}/comment`, {
        review: newComment,
        rating: 4
      });
          ...
          <ul className="flex flex-col-reverse">
            {comments.map((comment) => (
              <li key={comment.created_at} className="mb-4">
                <Button variant="ghost" disabled>
                    {comment.avatar ? (
                      <img src={comment.avatar} className="w-8 h-8 rounded-full" alt="User Avatar" />
                    ) : (
                      <User size={20} />
                    )}
                  </Button>
